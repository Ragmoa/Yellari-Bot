...
export default function yellariUTCCommand(interaction) {
    const embed = new EmbedBuilder();
    let hours24;
    let timeOption = interaction.options.get('time');
    let ampmOption = interaction.options.get('ampm');

    let suffix = '';
    if (ampmOption) {
        suffix = ampmOption.value;
    }
    if (timeOption) {
